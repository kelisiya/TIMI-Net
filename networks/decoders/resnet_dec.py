...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, upsample=None, norm_layer=None, large_kernel=False, enc_expansion=1):
        ...
        if self.stride > 1:
            self.conv1 = SpectralNorm(nn.ConvTranspose2d(inplanes * enc_expansion,
                                                         inplanes,
                                                         kernel_size=4,
                                                         stride=2,
                                                         padding=1,
...
class ResNet_D_Dec(nn.Module):
    ...
    def __init__(self, block, layers, enc_expansion=1, norm_layer=None,  large_kernel=False, late_downsample=False):
        super(ResNet_D_Dec, self).__init__()
        self.logger = logging.getLogger("Logger")

        def wrapped_norm_layer(*args, **kwargs):
            return nn.BatchNorm2d(*args, **kwargs, eps=1e-3)
        if norm_layer is None:
            # norm_layer = nn.BatchNorm2d
            norm_layer = wrapped_norm_layer
        self._norm_layer = norm_layer
        self.large_kernel = large_kernel
        self.kernel_size = 5 if self.large_kernel else 3
        self.enc_expansion = enc_expansion
        ...
        # self.conv1 = SpectralNorm(nn.Conv2d(32, 32, kernel_size=3, stride=1, padding=1, bias=False))
    ...
    def _make_layer(self, block, planes, blocks, stride=1):
        ...
        if stride != 1:
            upsample = nn.Sequential(
                ...
                SpectralNorm(conv1x1(self.inplanes * self.enc_expansion, planes * block.expansion)),
        ...
        layers = [block(self.inplanes, planes, stride, upsample, norm_layer, self.large_kernel, self.enc_expansion)]
